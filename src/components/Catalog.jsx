...
function Catalog({ onTagClick, filtersRef, tagFilter, resetTagFilter }) {
  ...
  const buildQuery = useCallback(() => {
    ...
    // Фильтры
    if (appliedFilters.price?.min) params.append('priceMin', appliedFilters.price.min);
    if (appliedFilters.price?.max) params.append('priceMax', appliedFilters.price.max);
    if (appliedFilters.area?.min) params.append('areaMin', appliedFilters.area.min);
    if (appliedFilters.area?.max) params.append('areaMax', appliedFilters.area.max);
    if (appliedFilters.delivery?.min) params.append('deliveryMin', appliedFilters.delivery.min);
    if (appliedFilters.delivery?.max) params.append('deliveryMax', appliedFilters.delivery.max);
    if (appliedFilters.floor?.min) params.append('floorMin', appliedFilters.floor.min);
    if (appliedFilters.floor?.max) params.append('floorMax', appliedFilters.floor.max);
    ['rooms','city','type','view','finishing','payment','sea-distance'].forEach(key => {
      if (Array.isArray(appliedFilters[key]) && appliedFilters[key].length > 0) {
        appliedFilters[key].forEach(val => params.append(key, val));
      }
    });
    // Сортировка
    ...
    <section className="bg-white min-h-screen p-6 animate-fadeIn" style={{ position: 'static' }}>
      ...
      {showScrollButton && (
        <button
          ...
          className="fixed bottom-6 right-6 z-50 bg-[var(--primary)] text-white p-3 rounded-full shadow-lg hover:bg-[var(--blue-700)] transition-all duration-300"
